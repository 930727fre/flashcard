...
export default function BatchAddPage() {
  const { gasUrl, clearLocalCache } = useStore();
  const navigate = useNavigate();
  const [loading, setLoading] = useState(false);
  const [content, setContent] = useState('');

  const handleBatchSubmit = async () => {
    if (!content.trim() || !gasUrl) return;
    setLoading(true);
    ...
    const newCards: Card[] = lines
      ...
      .map(line => {
        ...
        return {
          ...
          sentence: sentence || '',
          note: note || '',
          due: '', // 修正：新匯入單字不給 due，待領取時再由 GAS 設定
          stability: 0,
          difficulty: 0,
          elapsed_days: 0,
          scheduled_days: 0,
    ...
    try {
      ...
      clearLocalCache(); // Force re-fetch from GAS on next dashboard load
