...
export default function ReviewPage() {
  const { cards, settings, isLoading, updateCardInStore, updateSettingsInStore, syncToGAS } = useStore();
  ...
  const [isInitialized, setIsInitialized] = useState(false); // 修正 Bug 3: 防止假性空佇列
  // Tracks new cards reviewed this session to avoid stale closure on settings
  const newCountRef = useRef(Number(settings?.daily_new_count || 0));

  // Sync to GAS when the review session completes
  useEffect(() => {
    if (isInitialized && reviewQueue.length === 0) {
      syncToGAS();
    }
  }, [isInitialized, reviewQueue.length, syncToGAS]);

  // --- 初始化邏輯：只在資料到位後執行一次 ---
  useEffect(() => {
    // 只有在載入結束、cards 有資料、且還沒初始化過時才執行
    if (!isLoading && cards.length > 0 && !isInitialized) {
      const queue = computeQueue(cards, settings);
      setReviewQueue(queue);
      setTotalCount(queue.length);
      setIsInitialized(true);
    }
  }, [isLoading, cards, isInitialized]);
  ...
  const handleRate = useCallback((rating: Rating) => {
    ...
    const params = settings?.fsrs_params && settings.fsrs_params !== "undefined"
      ? JSON.parse(settings.fsrs_params)
      : generatorParameters();
    const nextFields = computeNext(currentCard, rating, params);

    // All updates are local (localStorage + Zustand); GAS sync happens on session end
    updateCardInStore(currentCard.id, nextFields);
    if (isNew) {
      newCountRef.current += 1;
      updateSettingsInStore({ daily_new_count: newCountRef.current });
    }

    setReviewQueue(prev => prev.slice(1));
    setIsFlipped(false);
  }, [currentCard, settings, updateCardInStore, updateSettingsInStore]);
  ...
  // 1. 載入中（包含等待初始化）
  if (isLoading || (!isInitialized && cards.length === 0)) {
    return (
